...
namespace QuadTree.Common
{
    public abstract class QuadTreeFNodeCommon
    {
        ...
        protected Rectangle2D Rect; // The area this QuadTree represents

        /// <summary>
        /// The area this QuadTree represents.
        /// </summary>
        internal virtual Rectangle2D QuadRect
        {
            get { return Rect; }
        }
    }
        ...
        where TNode : QuadTreeFNodeCommon<T, TNode, TQuery>
    {
        ...
        {
            get { return _childBr.Rect.Center; }
        }
        ...
        protected QuadTreeFNodeCommon(Rectangle2D rect)
        {
            Rect = rect;
        }
        ...
        internal QuadTreeFNodeCommon(TNode parent, Rectangle2D rect)
            : this(rect)
        {
            _parent = parent;
        }
        ...
        /// </summary>
        internal Point2D Subdivide(bool recursive = true)
        {
            float area = (float)(Rect.Width*Rect.Height);
            ...
            Point2D mid = new Point2D(Rect.X + (Rect.Width/2), Rect.Y + (Rect.Height/2));
        ...
        /// </summary>
        public void Subdivide(Point2D mid, bool recursive = true)
        {
            Debug.Assert(_childTl == null);
            // We've reached capacity, subdivide...
            _childTl = CreateNode(new Rectangle2D(Rect.Left, Rect.Top, mid.X - Rect.Left, mid.Y - Rect.Top));
            _childTr = CreateNode(new Rectangle2D(mid.X, Rect.Top, Rect.Right - mid.X, mid.Y - Rect.Top));
            _childBl = CreateNode(new Rectangle2D(Rect.Left, mid.Y, mid.X - Rect.Left, Rect.Bottom - mid.Y));
            _childBr = CreateNode(new Rectangle2D(mid.X, mid.Y, Rect.Right - mid.X, Rect.Bottom - mid.Y));
        ...
        protected void VerifyNodeAssertions(Rectangle2D Rectangle2D)
        {
            Debug.Assert(Rectangle2D.Width > 0);
            Debug.Assert(Rectangle2D.Height > 0);
        }

        protected abstract TNode CreateNode(Rectangle2D Rectangle2D);
        ...
        private UInt32 MortonIndex2(Point2D Point2D, float minX, float minY, float width, float height)
        {
            Point2D = new Point2D(Point2D.X - minX, Point2D.Y - minY);
            var pX = (UInt32) (UInt16.MaxValue*Point2D.X/width);
            var pY = (UInt32) (UInt16.MaxValue*Point2D.Y/height);

            return EncodeMorton2(pX, pY);
        }

        protected abstract Point2D GetMortonPoint(T p);

        internal void InsertStore(Point2D tl, Point2D br, T[] range, int start, int end,
            Func<T, QuadTreeObject<T, TNode>> createObject, int threadLevel, List<Task> tasks = null)
        {
            ...
            var count = end - start;
            float area =(float)( (br.X - tl.X)*(br.Y - tl.Y));
            if (count > 8 && area > 0.01f && !float.IsInfinity(area))
            {
                ...
                Point2D middlePoint = GetMortonPoint(range[quater2]);
                ...
                else
                {
                    ...
                    Debug.Assert(!float.IsNaN((float)middlePoint.X));
                }

                if (threadLevel == 0)
                {
                    ChildTl.InsertStore(tl, middlePoint, range, start, quater1, createObject, 0);
                    ChildTr.InsertStore(new Point2D(middlePoint.X, tl.Y), new Point2D(br.X, middlePoint.Y), range, quater1,
                        quater2, createObject, 0);
                    ChildBl.InsertStore(new Point2D(tl.X, middlePoint.Y), new Point2D(middlePoint.X, br.Y), range, quater2,
                ...
                else
                {
                    ...
                    if (--threadLevel == 0)
                    {
                        ...
                        tasks.Add(
                            Task.Run(
                                () =>
                                    ChildTr.InsertStore(new Point2D(middlePoint.X, tl.Y),
                                        new Point2D(br.X, middlePoint.Y), range, quater1,
                                        quater2, createObject, 0)));
                        tasks.Add(
                            Task.Run(
                                () =>
                                    ChildBl.InsertStore(new Point2D(tl.X, middlePoint.Y),
                                        new Point2D(middlePoint.X, br.Y), range, quater2,
                                        quater3, createObject, 0)));
                        tasks.Add(
                            Task.Run(
                                () => ChildBr.InsertStore(middlePoint, br, range, quater3, end, createObject, 0)));
                    ...
                    else
                    {
                        ...
                        ChildTr.InsertStore(new Point2D(middlePoint.X, tl.Y), new Point2D(br.X, middlePoint.Y), range,
                            quater1,
                            quater2, createObject, threadLevel, tasks);
                        ChildBl.InsertStore(new Point2D(tl.X, middlePoint.Y), new Point2D(middlePoint.X, br.Y), range,
                            quater2,
        ...
        public void AddBulk(T[] points, Func<T, QuadTreeObject<T, TNode>> createObject, int threadLevel = 0)
        {
                ...
                if (points.Length > 0)
                {
                    Parallel.ForEach(Partitioner.Create(0, points.Length), (a) =>
                    {
                        ...
                        for (int i = a.Item1; i < a.Item2; i++)
                        {
                            ...
                            {
                                localMaxX = (float)point.X;
                            }
                            if (point.X < localMinX)
                            {
                                localMinX = (float)point.X;
                            }
                            if (point.Y > localMaxX)
                            {
                                localMaxX = (float)point.Y;
                            }
                            if (point.Y < localMinY)
                            {
                                localMinY = (float)point.Y;
                            }
                        }
            ...
            InsertStore(QuadRect.UpperLeftPoint, new Point2D(QuadRect.Bottom, QuadRect.Right), range, 0, range.Length,
        ...
        protected abstract bool CheckContains(Rectangle2D Rectangle2D, T data);


        /// <summary>
        /// Get the objects in this tree that intersect with the specified rectangle.
        /// </summary>
        /// <param name="searchRect">The Rectangle2D to find objects in.</param>
        ...
        protected abstract bool QueryContains(TQuery search, Rectangle2D rect);
        protected abstract bool QueryIntersects(TQuery search, Rectangle2D rect);
        ...
        public IEnumerable<T> EnumObjects(TQuery searchRect)
        {
            ...
            do
            {
                ...
                else if (QueryIntersects(searchRect, node.Rect))
                {
                    ...
                    // Get the objects for the search Rectangle2D from the children
                    if (node.ChildTl != null)
                    {
        ...
        private Point2D CalculateBalance(out float top, out float bottom, out float left, out float right)
        {
            ...
            return new Point2D(xBalance, yBalance);
        }
    }


    public abstract class QuadTreeFNodeCommon<T, TNode> : QuadTreeFNodeCommon<T, TNode, Rectangle2D>
        where TNode : QuadTreeFNodeCommon<T, TNode>
    {
        protected QuadTreeFNodeCommon(Rectangle2D rect) : base(rect)
        {
        }
        ...
        public QuadTreeFNodeCommon(TNode parent, Rectangle2D rect) : base(parent, rect)
        {
        }
        ...
        public void Rebalance()
        {
            ...
            if (xVy > 0)
            {
                ...
                do
                {
                    if (inv)
                    {
                        searchRect = new Rectangle2D(QuadRect.X, QuadRect.Y - incrementer, QuadRect.Width, incrementer);
                    }
                    else
                    {
                        searchRect = new Rectangle2D(QuadRect.X, QuadRect.Y, QuadRect.Width, incrementer);
                    }
    ...
    public abstract class QuadTreeNodeFCommonPoint<T, TNode> : QuadTreeFNodeCommon<T, TNode, Point2D>
        where TNode : QuadTreeFNodeCommon<T, TNode, Point2D>
    {
        protected QuadTreeNodeFCommonPoint(Rectangle2D rect)
            : base(rect)
        {
        }
        ...
        public QuadTreeNodeFCommonPoint(TNode parent, Rectangle2D rect)
            : base(parent, rect)
        {
        }


        protected override bool QueryContains(Point2D search, Rectangle2D rect)
        {
            return rect.Contains(search);
        }

        protected override bool QueryIntersects(Point2D search, Rectangle2D rect)
        {
            return false;
        }
    }
