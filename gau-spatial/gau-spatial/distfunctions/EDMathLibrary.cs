    ...
    class EDMathLibrary
    {
        ...
        public static EDVector3D BresenhamLineStepping(EDVector3D actorpos, EDVector3D recieverpos, MapLevel maplevel)
        {
            ...
            for (int i = 1; i < el; i++) // Iterate every point on the LOS
            {
                ...
                stepcount++;
                // X and Y mark now a point on the LOS -> get Grid to that point and test for intersection
                MapgridCell celltest = grid[getGridPosX(x)][getGridPosY(y)];
                Console.WriteLine(x + " " + y + " has cell " + celltest);
